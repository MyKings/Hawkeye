<template>
  <div class="setting-card">
    ...
    <el-dialog title="批量添加监控项" :visible.sync="dialogFormVisibleBatch" v-model="dialogFormVisibleBatch"
               :width="mobileClient?'80%':'50%'">
      <el-form :model="form">
        <el-form-item label="关键字">
          <el-popover
          ...
          <el-input type="textarea" required="required" v-model="form.keyword" auto-complete="off" :rows="4"
                    placeholder="多个关键字请使用换行分割"></el-input>
        </el-form-item>
        <el-form-item label="开启/关闭">
          <el-switch
              v-model="form.enabled"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button round size="mini" @click="dialogFormVisibleBatch = false">取 消</el-button>
        <el-button round size="mini" type="primary" @click="handleBatchAddQuery(form)">确 定</el-button>
      </div>
    </el-dialog>
    ...
    <el-button type="warning" size="mini" @click="dialogFormVisibleBatch = true">
      批量添加
    </el-button>
...
export default {
  data() {
    return {
      rules: {
        keyword: [{required: true, message: '请输入名字'}]
      },
